...
internal class VRageSystem(string applicationName, VRageWindowSurrogate? surrogate, string? appdataPath) : IVRageSystem
{
    ...
    {
#if WINDOWS
        get => surrogate is null ? string.Empty : surrogate.Window.Invoke(System.Windows.Forms.Clipboard.GetText);
        set
        {
            surrogate?.Window.Invoke(() => System.Windows.Forms.Clipboard.SetText(value));
        }
#else
